...
def test_login_links_guest_chat(client):
    ...
    # Create customer account
    conn.execute("INSERT INTO users (id, name, email, google_id) VALUES (42, 'Google User', 'guser@example.com', 'google123')")
    conn.commit()
    conn.close()
    ...
    with mock.patch("services.oauth.exchange_google_code", return_value={"access_token": "token"}), \
         mock.patch("services.oauth.get_google_user_info", return_value={"sub": "google123", "email": "guser@example.com", "name": "Google User"}):
...
def test_customer_edit_restrictions(client):
    ...
    conn.execute(
        """
        INSERT INTO reservations (id, user_id, date, room_id, start_time, end_time, contact_name, contact_phone, contact_email, num_people, language, status, total_cost, deposit_paid)
        VALUES (99, 10, '2026-07-01', 1, '12:00', '13:00', 'Owner', '555-1234', 'owner@example.com', 2, 'en', 'pending', 40.0, 0.0)
        """
    )
    conn.commit()
    conn.close()
    ...
    resp = client.patch(
        "/api/reservations/99",
        data=json.dumps({"total_cost": 50.0}),
        content_type="application/json"
    )
    assert resp.status_code == 400
