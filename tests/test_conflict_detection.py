...
def make_reservation(conn, **kwargs):
    ...
    cur = conn.execute(
        '''INSERT INTO reservations
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
        (
            defaults['date'], defaults['start_time'], defaults['end_time'],
            defaults['num_people'], defaults['contact_name'], defaults['contact_phone'],
            defaults['contact_email'], defaults['room_id'], defaults['language'],
            defaults['status'], defaults['total_cost'], defaults['notes']
        )
    )
    conn.commit()
