...
def _blackout_windows():
    windows = current_app.config.get('BLACKOUT_WINDOWS', [])
...
def create_reservation_api_payload(
    ...
    *,
    ...
    required_fields = ['date', 'start_time', 'end_time', 'num_people', 'contact_name', 'contact_phone', 'room_id']
    ...
    try:
        ...
        return api_error(str(e), 400, code='validation_error', fields=['date', 'start_time', 'end_time'])
...
def update_reservation_api_payload(reservation_id, data, api_error, api_ok):
    ...
    try:
        ...
        return api_error(str(e), 400, code='validation_error', fields=['start_time', 'end_time', 'date'])
    ...
    time_or_room_changed = (
        normalized_start != existing['start_time'] or
        normalized_end != existing['end_time'] or
    ...
    return api_ok({'reservation': serialize_reservation_row(updated)}, message='Reservation updated')
