...
def api_daily_reservations():
    ...
    if not date:
        return api_error(
            "Date parameter is required", 400, code="validation_error", fields=["date"]
        )
...
def api_get_reservation(reservation_id):
    ...
    return api_ok(
        {"reservation": serialize_reservation_row(row, row["id"] in idle_set)}
    )
...
def api_update_reservation_route(reservation_id):
    return update_reservation_api_payload(
        reservation_id, request_payload(), api_error, api_ok
    )
...
def api_calendar_availability():
    ...
    try:
        ...
        return api_error(
            "Invalid date format", 400, code="validation_error", fields=["start", "end"]
        )
...
def price_estimate():
    ...
    try:
        ...
        return api_error(
            str(e), 400, code="validation_error", fields=["start_time", "end_time"]
        )
