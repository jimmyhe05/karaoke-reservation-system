...
from services.pricing import (
    compute_pricing as compute_pricing_service,
    calculate_cost as calculate_cost_service,
)
...
def seed_sample_data(target_date=None):
    ...
    for s in samples:
        total_cost = calculate_cost_service(
            get_db(), s["room_id"], s["start_time"], s["end_time"], TAX_RATE
        )
        conn.execute(
...
@click.option(
    "--date", default=None, help="YYYY-MM-DD date to seed (defaults to today)"
)
...
def compute_pricing(conn, room_id, start_time_str, end_time_str):
    return compute_pricing_service(
        conn, room_id, start_time_str, end_time_str, TAX_RATE
    )
...
def is_within_business_hours(start_time, end_time):
    ...
    return 11.0 <= normalized_start < 25.0 and 11.0 < normalized_end <= 25.0
...
def get_daily_reservations():
    ...
    if idle_reservation_ids_set:
        idle_reservations = db.execute(
            ...
            ORDER BY start_time
        """.format(
                ",".join(["?"] * len(idle_reservation_ids_set))
            ),
            list(idle_reservation_ids_set) + [date],
        ).fetchall()
...
def delete_reservation(reservation_id):
    ...
    try:
        ...
        conn.execute(
            "DELETE FROM reservation_history WHERE reservation_id = ?",
            (reservation_id,),
        )
        ...
        return (
            jsonify(
                {"message": "Reservation deleted successfully", "id": reservation_id}
            ),
            200,
        )
...
def update_reservation(reservation_id):
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": str(e), "fields": ["start_time", "end_time", "date"]}
                ),
                400,
            )
        ...
        if (
            normalized_start != existing_reservation["start_time"]
            or normalized_end != existing_reservation["end_time"]
            or room_id != existing_reservation["room_id"]
            or date != existing_reservation["date"]
        ):
        ...
        contact_name = data.get("contact_name", existing_reservation["contact_name"])
        contact_phone = data.get("contact_phone", existing_reservation["contact_phone"])
        contact_email = data.get("contact_email", existing_reservation["contact_email"])
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": "Invalid number of people", "fields": ["num_people"]}
                ),
                400,
            )
        ...
        if not ok:
            return jsonify({"error": msg, "fields": ["room_id", "num_people"]}), 400
...
def move_to_idle(reservation_id):
    ...
    try:
        ...
        if existing:
            ...
            return (
                jsonify(
                    {"success": True, "message": "Reservation already in idle area"}
                ),
                200,
            )
...
def move_reservation():
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": "Invalid reservation id", "fields": ["reservation_id"]}
                ),
                400,
            )
        ...
        try:
            ...
            try:
                ...
                return (
                    jsonify({"error": str(e), "fields": ["start_time", "end_time"]}),
                    400,
                )

            blocked, win = is_blackout(
                date, new_start_time_str, new_end_time_str, room_id
            )
            if blocked:
                return api_error(
...
def today_stats():
    ...
    return jsonify(
        {"total_reservations": total_reservations, "occupancy_rate": occupancy_rate}
    )
...
def price_estimate():
    ...
    try:
        ...
        return (
            jsonify({"error": str(e), "fields": ["date", "start_time", "end_time"]}),
            400,
        )

    try:
        pricing = compute_pricing(
            get_db(), data["room_id"], normalized_start, normalized_end
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def room_suggestion():
    ...
    try:
        ...
        return jsonify({"suggested_rooms": suggested_rooms, "reason": reason})
    except Exception as e:
        return jsonify({"error": str(e)}), 400
        ...
        while current_time < end_time:
            ...
            for reservation in existing_reservations:
                if (
                    ...
                    and current_time < reservation["end_time"]
                ):
                    is_available = False
                    break
            ...
            current_time = datetime.strptime(
                f"{current_hour:02d}:{current_minute:02d}", "%H:%M"
            ).time()

        return jsonify({"alternatives": alternatives[:5]})  # Return top 5 alternatives
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def admin_login():
    ...
    if (
        ...
        role = "admin"
    elif username == app.config.get("STAFF_USERNAME") and password == app.config.get(
        "STAFF_PASSWORD"
    ):
        role = "staff"
